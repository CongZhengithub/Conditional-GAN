...
class Image_data:
    ...
    def __init__(self, img_height, img_width, channels, segmap_ch, dataset_path, augment_flag):
        ...
        self.img_dataset_path = os.path.join(dataset_path, 'images/training')
        self.segmap_dataset_path = os.path.join(dataset_path, 'annotations/training')
        self.segmap_test_dataset_path = os.path.join(dataset_path, 'annotations/validation')
    ...
    def image_processing(self, filename, segmap):
        ...
        segmap_decode = tf.image.decode_png(segmap_x, channels=self.segmap_channel)#, dct_method='INTEGER_ACCURATE'
