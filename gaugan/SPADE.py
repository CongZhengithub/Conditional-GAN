...
class SPADE(object):
    def __init__(self, sess, args):
        ...
        self.random_x=tf.random_normal(shape=[1, self.ch * 4])
    ...
    def generator(self, segmap, x_mean, x_var, random_style=False, reuse=False, scope="generator"):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            if random_style :
                x = self.random_x#tf.random_normal(shape=[batch_size, self.ch * 4])
            else :
    ...
    def random_test(self):
        ...
        segmap_files = glob('./dataset/{}/{}/{}/*.*'.format(self.dataset_name, 'annotations','validation'))
